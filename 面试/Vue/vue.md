...
<font size=4>&emsp;&emsp;v-show 是对元素的 display 属性进行操作，最终控制元素是否显示.不管元素是否显示，该元素节点都会一直存在于 dom 结构上.在平常使用时，适合用在频繁切换的 dom 上，能够提高性能。

&emsp;&emsp;v-if 是条件渲染，虽然同是控制显示和隐藏，和 v-show 有着本质上的区别，值为 false 的情况下，该 dom 不会被渲染，只有值为 true 的时候 dom 才会渲染。适合用在非频繁切换显示隐藏的情况下，正常使用这个会更保险一点，使用 v-show 有时候有一些莫名其妙的的 bug。
</font>
...
&emsp;&emsp;computed 是计算属性，就是通过一些计算后，得到另外的属性。比如我们需要依赖别的属性来获取动态值的时候就可以使用计算属性，一般对同步数据进行处理.我们一般使用的计算属性的 get 方法（简写），当我们想改变计算属性时，我们需要添加 set 方法，通过 set 方法，修改原始值，在观察到原始值变化后，更新计算属性值，计算属性是惰性的。

&emsp;&emsp;watch 就是监听，但我们想要在一个属性或者对象发生改变时，做一些逻辑操作，我们就可以使用监听。监听可以是异步的，使用 immediate 时，watch 就变成非惰性的了，页面挂载就会执行相应的副作用。
...
接口权限就是对用户的检验.正常来说，用户登录会返回一个 token，然后每次调用接口时，都会携带这个 token，然后服务器通过对比 token，如果通过则可以进行访问

##### 3 页面权限控制

-1 创建路由表，因为有部分页面访问是需要有权限的，我们可以将这些有访问权限的路由(异步路由)单独放到一个变量或者文件中，方便后期的代码维护

-2 页面访问权限

- 通过请求获取用户权限列表
- 进行页面匹配，通过请求回来的用户权限列表和自己创建的权限列表，
- 编写全局前置守卫导航钩子，通过调用 router.addRoutes()方法，将匹配到的权限列表添加到路由器中，最终页面能够进行访问

-3 数据操作权限

- 可以自定义指令，然后通过判断当前路由组件中是否有对应权限的，从而判断对应按钮的显示与隐藏

##### 路由控制完整流程图

<img src="https://user-gold-cdn.xitu.io/2019/9/4/16cfa3fd18b05c8a?imageView2/0/w/1280/h/960/format/webp/ignore-error/1" width="500" height="800" />
